...
x_train = np.loadtxt(path+'x_train_mean1.txt', delimiter=',', dtype='float')
y_train = np.loadtxt(path+'y_train_mean1.txt', delimiter=',', dtype='float')
x_test = np.loadtxt(path+'x_test_mean1.txt', delimiter=',', dtype='float')
...
xlf = xgb.XGBRegressor(max_depth=10, learning_rate=0.01, n_estimators=160, subsample=0.8, silent=False,
...
np.savetxt(save_path+'prediction_mean7.txt', prediction, fmt='%.3e',  delimiter=',')