...
# train
gbm0 = lgb.LGBMRegressor(objective='regression', num_leaves=33, learning_rate=0.1, n_estimators=150,
                         min_child_weight=1, colsample_bytree=0.7, reg_lambda=3, reg_alpha=0.1)
gbm0.fit(x_train, y_train[:, 0], eval_metric=evalerror)
pred0 = gbm0.predict(x_test)
prediction = pred0

gbm1 = lgb.LGBMRegressor(objective='regression', num_leaves=39, learning_rate=0.1, n_estimators=80,
                         min_child_weight=1, colsample_bytree=0.7, reg_lambda=3, reg_alpha=0.1)
gbm1.fit(x_train, y_train[:, 1], eval_metric=evalerror)
pred1 = gbm1.predict(x_test)
prediction = np.vstack((prediction, pred1))

gbm2 = lgb.LGBMRegressor(objective='regression', num_leaves=30, learning_rate=0.1, n_estimators=80,
...
gbm3 = lgb.LGBMRegressor(objective='regression', num_leaves=47, learning_rate=0.1, n_estimators=230,
                         min_child_weight=1, colsample_bytree=0.7, reg_lambda=3, reg_alpha=2)
gbm3.fit(x_train, y_train[:, 3], eval_metric=evalerror)
pred3 = gbm3.predict(x_test)
prediction = np.vstack((prediction, pred3))

gbm4 = lgb.LGBMRegressor(objective='regression', num_leaves=51, learning_rate=0.05, n_estimators=150,
                         min_child_weight=1, colsample_bytree=0.7, reg_lambda=3, reg_alpha=2)
...
np.savetxt(save_path+'prediction_gbm_nan_change_error31.txt', prediction, fmt='%.3e',  delimiter=',')
